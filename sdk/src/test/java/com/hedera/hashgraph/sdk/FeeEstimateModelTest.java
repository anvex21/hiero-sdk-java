...
class FeeEstimateModelTest {
    ...
    void feeEstimateResponseRoundTrip() throws InvalidProtocolBufferException {
        ...
        // Using equals() returns the same output but the test doesn't pass because of different object references (hash
        // codes)
        ...
        // Compare network
        assertThat(responseFromBytes.getNetwork().getMultiplier())
                .isEqualTo(response.getNetwork().getMultiplier());
        assertThat(responseFromBytes.getNetwork().getSubtotal())
                .isEqualTo(response.getNetwork().getSubtotal());

        // Compare node
        assertThat(responseFromBytes.getNode().getBase())
                .isEqualTo(response.getNode().getBase());
        assertThat(responseFromBytes.getNode().getExtras())
                .hasSameSizeAs(response.getNode().getExtras());

        // Compare service
        assertThat(responseFromBytes.getService().getBase())
                .isEqualTo(response.getService().getBase());
        assertThat(responseFromBytes.getService().getExtras())
                .hasSameSizeAs(response.getService().getExtras());
    }
}
